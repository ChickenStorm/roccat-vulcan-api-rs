//! Contains the constant reports

/// First report for the control device
const FEATURE_REPORT_1: [u8; 3] = [0x15, 0x00, 0x01];
/// Second report for the control device
const FEATURE_REPORT_2: [u8; 4] = [0x05, 0x04, 0x00, 0x04];
/// Third report for the control device
const FEATURE_REPORT_3: [u8; 95] = [
    0x07, 0x5f, 0x00, 0x3a, 0x00, 0x00, 0x3b, 0x00, 0x00, 0x3c, 0x00, 0x00, 0x3d, 0x00, 0x00, 0x3e,
    0x00, 0x00, 0x3f, 0x00, 0x00, 0x40, 0x00, 0x00, 0x41, 0x00, 0x00, 0x42, 0x00, 0x00, 0x43, 0x00,
    0x00, 0x44, 0x00, 0x00, 0x45, 0x00, 0x00, 0x46, 0x00, 0x00, 0x47, 0x00, 0x00, 0x48, 0x00, 0x00,
    0xb3, 0x00, 0x00, 0xb4, 0x00, 0x00, 0xb5, 0x00, 0x00, 0xb6, 0x00, 0x00, 0xc2, 0x00, 0x00, 0xc3,
    0x00, 0x00, 0xc0, 0x00, 0x00, 0xc1, 0x00, 0x00, 0xce, 0x00, 0x00, 0xcf, 0x00, 0x00, 0xcc, 0x00,
    0x00, 0xcd, 0x00, 0x00, 0x46, 0x00, 0x00, 0xfc, 0x00, 0x00, 0x48, 0x00, 0x00, 0xcd, 0x0e,
];
/// Forth report for the control device
const FEATURE_REPORT_4: [u8; 8] = [0x0a, 0x08, 0x00, 0xff, 0xf1, 0x00, 0x02, 0x02];
/// Fifth report for the control device
const FEATURE_REPORT_5: [u8; 8] = [0x0a, 0x08, 0x00, 0xff, 0xf1, 0x00, 0x02, 0x02];
/// Sixth report for the control device
const FEATURE_REPORT_6: [u8; 133] = [
    ,
];
/// Seventh report for the control device
const FEATURE_REPORT_7: [u8; 43] = [
    0x09, 0x2b, 0x00, 0x49, 0x00, 0x00, 0x4a, 0x00, 0x00, 0x4b, 0x00, 0x00, 0x4c, 0x00, 0x00, 0x4d,
    0x00, 0x00, 0x4e, 0x00, 0x00, 0xa4, 0x00, 0x00, 0x8e, 0x00, 0x00, 0xd0, 0x00, 0x00, 0xd1, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0xcd, 0x04,
];
/// Eight report for the control device, this is the rainbow one
const FEATURE_REPORT_8: [u8; 443] = [
    ,
];
/// Eight report for the control device, alternative version
const FEATURE_REPORT_8_ALT: [u8; 443] = [
    ,
];
/// Nineth report for the control device
const FEATURE_REPORT_9: [u8; 8] = [0x13, 0x08, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00];

/// List of feature report for the control device on the default rainbow mode.
pub(crate) const FEATURE_REPORT_RAINBOW: [&[u8]; 9] = [
    &FEATURE_REPORT_1,
    &FEATURE_REPORT_2,
    &FEATURE_REPORT_3,
    &FEATURE_REPORT_4,
    &FEATURE_REPORT_5,
    &FEATURE_REPORT_6,
    &FEATURE_REPORT_7,
    &FEATURE_REPORT_8,
    &FEATURE_REPORT_9,
];
/// List of feature report for the control device on the custom mode.
pub(crate) const FEATURE_REPORT_CUSTOM: [&[u8]; 9] = [
    &FEATURE_REPORT_1,
    &FEATURE_REPORT_2,
    &FEATURE_REPORT_3,
    &FEATURE_REPORT_4,
    &FEATURE_REPORT_5,
    &FEATURE_REPORT_6,
    &FEATURE_REPORT_7,
    &FEATURE_REPORT_8_ALT,
    &FEATURE_REPORT_9,
];

/// header for the led write buffer
pub(crate) const LED_FEATURE_REPORT_HEAD: [u8; 4] = [0xa1, 0x01, 0x01, 0xb4];
